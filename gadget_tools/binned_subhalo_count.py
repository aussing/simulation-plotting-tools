...
def plot_stellar(total_mass, mass_type, total_len, part_num_cut,radius,color,label,linestyle,fig, ax,main_halo_only=True,alpha=1):
    ...
    # num_bins = 5
    # bin_width_estimate = 0.5
    # bins_min, bins_max = np.log10(5.5e5), np.log10(8e8) # np.min(stellar_mass)-0.05*np.min(stellar_mass), np.max(stellar_mass)+0.05*np.min(stellar_mass)
    # num_bins = int((bins_max-bins_min)/bin_width_estimate)
    # print(num_bins)
    # bins = np.linspace(bins_min,bins_max,num_bins)
    # binned_sats,bin_edges = np.histogram(np.log10(stellar_mass),bins=bins)

    bins_min, bins_max = 5.5e5, 8e8
    bin_width_estimate = 0.5
    num_bins = int((np.log10(bins_max)-np.log10(bins_min))/bin_width_estimate)
    print(num_bins)
    bins = np.geomspace(bins_min,bins_max,num_bins)
    binned_sats,bin_edges = np.histogram(stellar_mass,bins=bins)
    ...
    for i in range(len(bin_edges)-1):
        ...
        bin_width.append(bin_edges[i+1]-bin_edges[i])
        
    bin_centres = np.array(bin_centres)
    bin_width = np.array(bin_width)
    

    plt.plot(bin_centres,binned_sats/(bin_width),color=color,label=label,linestyle=linestyle,alpha=alpha)
    plt.xscale('log')
    plt.yscale('log')
    # plt.ylim((0.5,5))
    plt.xlabel(r'M$_{*}$ [M$_{\odot}$]')
    plt.ylabel(r'dn/dM$_{*}$ [M$_{\odot}$ dex$^{-1}$]')
    
    plt.legend(loc='lower left')
    plt.title(f'Stellar MW Halo - min part = {part_num_cut}')
    plt.savefig(f'./binned_sat_counts_{part_num_cut}.png',dpi=400)
    print(f'./binned_sat_counts_{part_num_cut}.png')
...
def get_obs_mass(normalise=False):
    classical = ['LMC','SMC','Fornax','Leo1','Sculptor','Leo2','SagittariusdSph','Sextans(1)','Carina','Draco','UrsaMinor']
    classical_mass = 1.5e9,4.8e8,2.0e7,5.5e6,2.3e6,7.4e6,2.1e7,4.4e5,3.8e5,2.9e5#,1.4e4 #from Lovell 2023
...
def subhalo_data(sim_directory,i_file,main_halo_only=True):
    ...
    if main_halo_only:
        ...
        print()
    return subhalo_mass, subhalo_mass_type, subhalo_len, subhalo_len_type, halo_R200c[mass_mask[halo_mainID]]
...
def main_halo_plots(folder,i_file,part_num_cut):
    ...
    main_halo_only = True
    print("--- cdm_sn_005 ---")
    total_mass_cdm_sn_005, mass_type_cdm_sn_005, total_len_cdm_sn_005, len_type_cdm_sn_005, r_200_cdm_sn_005 = subhalo_data(cdm_sn_005,i_file,main_halo_only)
    print("--- cdm_sn_010 ---")
    total_mass_cdm_sn_010, mass_type_cdm_sn_010, total_len_cdm_sn_010, len_type_cdm_sn_010, r_200_cdm_sn_010 = subhalo_data(cdm_sn_010,i_file,main_halo_only)
    print("--- wdm_sn_005 ---")
    total_mass_wdm_sn_005, mass_type_wdm_sn_005, total_len_wdm_sn_005, len_type_wdm_sn_005, r_200_wdm_sn_005 = subhalo_data(wdm_sn_005,i_file,main_halo_only)
    print("--- wdm_sn_010 ---")
    total_mass_wdm_sn_010, mass_type_wdm_sn_010, total_len_wdm_sn_010, len_type_wdm_sn_010, r_200_wdm_sn_010 = subhalo_data(wdm_sn_010,i_file,main_halo_only)
    print()
    ...
    # plot_stellar(total_mass_cdm_sn_005, mass_type_cdm_sn_005, total_len_cdm_sn_005, part_num_cut, r_200_cdm_sn_005, 'blue',  'CDM SN=5\%' , '-.', fig, ax, main_halo_only, alpha=1)
    plot_stellar(total_mass_cdm_sn_010, mass_type_cdm_sn_010, total_len_cdm_sn_010, part_num_cut, r_200_cdm_sn_010, 'blue',  'CDM SN=10\%', '-' , fig, ax, main_halo_only, alpha=1)
    plot_stellar(total_mass_wdm_sn_005, mass_type_wdm_sn_005, total_len_wdm_sn_005, part_num_cut, r_200_wdm_sn_005, 'orange','WDM SN=5\%' , '-.', fig, ax, main_halo_only, alpha=1)
    # plot_stellar(total_mass_wdm_sn_010, mass_type_wdm_sn_010, total_len_wdm_sn_010, part_num_cut, r_200_wdm_sn_010, 'orange','WDM SN=10\%', '-' , fig, ax, main_halo_only, alpha=1)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(f'./{folder}/subhalo/'):
        os.makedirs(f'./{folder}/subhalo/')
    
    plt.close()
    part_num_cut=[20]#,100,200]
    for num,part_num in enumerate(part_num_cut):
        main_halo_plots(folder,i_file,part_num)
        # population_plots(folder,i_file,part_num)
