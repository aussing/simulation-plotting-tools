...
def plot_stellar(total_mass, mass_type, total_len, part_num_cut,z,color,label,linestyle,fig, ax,main_halo_only=True,alpha=1):
    ...
    plt.plot(stellar_mass,stellar_mass_sum,color=color,label=label,linestyle=linestyle,alpha=alpha)
    if main_halo_only:
        plt.xlim((2e5,5e9))
        plt.ylim((9e-1,15))
        # plt.title(f'Satellite Galaxies of Main Halo at z={np.round(z,2)}')#, Min particles = {part_num_cut}')
    # else:
        # plt.title(f'Satellite Galaxy Population at z={np.round(z,2)}')#, Min particles = {part_num_cut}')
    # plt.xlim((2e5,5e9))
    # plt.ylim((9e-1,15))
    # plt.ylim((9e-1,30))
    ...
    if main_halo_only:
        ...
        # plt.savefig(f'./test_plots/stellar_mass_main_halo_part_cut_{part_num_cut}.png',dpi=400)
        plt.text(0.95,0.95,"Milky Way halo only",ha='right', va='top',transform=ax.transAxes,fontsize=40.0,fontweight='heavy')
        # plt.savefig(f'{folder}/subhalo/stellar_mass_main_halo_part_cut_{part_num_cut}.png',dpi=400)
        # plt.savefig(f'./stellar_mass_main_halo_part_cut_{part_num_cut}.png',dpi=400)
    else:
        # plt.title(f'Stellar Pop - min part = {part_num_cut}')
        
        plt.text(0.95,0.95,"Full simulation",ha='right', va='top',transform=ax.transAxes,fontsize=40.0,fontweight='heavy')
        plt.tight_layout()
        plt.legend(loc='lower left')
        plt.savefig(f'./stellar_mass_pop_part_cut_{part_num_cut}_all_sims.png',dpi=400)
        # plt.savefig(f'{folder}/subhalo/stellar_mass_population_part_cut_{part_num_cut}.png',dpi=400)
