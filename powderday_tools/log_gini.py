...
def gini(input_data):
    ...
    flattened = flattened[np.isfinite(np.log10(flattened))]
    # print(flattened)
    print(flattened.shape)
...
def rotate_and_mask(halo_pos,extent,exclusion,dust_coords,roation_maxtrix,axis):
    ...
    if axis == 'z':
        dist_axis = np.sqrt((rot_pos[:,0]-halo_pos_rot[0])**2+(rot_pos[:,1]-halo_pos_rot[1])**2) 
        dist_mask = (dists_rot<=extent) & (dist_axis>=exclusion)  
    if axis == 'y':
        dist_axis = np.sqrt((rot_pos[:,0]-halo_pos_rot[0])**2+(rot_pos[:,2]-halo_pos_rot[2])**2)
        dist_mask = (dists_rot<=extent) & (dist_axis>=exclusion) 
    if axis == 'x':
        dist_axis = np.sqrt((rot_pos[:,1]-halo_pos_rot[1])**2+(rot_pos[:,2]-halo_pos_rot[2])**2)
        dist_mask = (dists_rot<=extent) & (dist_axis>=exclusion) 
...
def gini_dust(folder,snap_list,num_bins,out_folder,rotations,softening):
    ...
    for i_snap in trange(len(snap_list)):
        ...
        # print(np.array(gini_score_cdm_sn_05_xy).shape)
...
def gini_history(folder,out_folder):
    ...
    num_bins = 80
    rotations = 1
    # softening_list = [0.0,2.5,15.0,25.0,30.0]
    softening_list = [15.0]
...
if __name__ =='__main__':
    ...
    # folder_list = ['N2048_L65_sd17492', 'N2048_L65_sd28504', 'N2048_L65_sd34920', 'N2048_L65_sd57839', 'N2048_L65_sd61284', 'N2048_L65_sd70562', 'N2048_L65_sd93745']
    
    out_folder = f'./test_plots/gini/{folder}/dtm/'
