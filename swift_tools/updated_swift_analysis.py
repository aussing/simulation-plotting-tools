...
CONFIG = {
    ...
    # 'cdm_path': '/fred/oz390/L12pt5/N188/',
    # 'wdm_path': '/fred/oz381/cpower/L12pt5N188_WDM0pt5/',
...
def process_halo_data(dm_type, snapshot, halo_pos, halo_rad):
    ...
    try:
        ...
        return gas_coords, gas_mass, dust_mass, avg_metallicity
...
def create_dust_map(coords, dust_mass, halo_rad, bin_size, output_path, title=None):
    ...
    # Set matplotlib parameters for smaller fonts
    plt.rcParams.update({
    ...
    #     'figure.titlesize': 10    # Figure title size
        ...
        'font.size': 15,           # Smaller base font size
    ...
    # Create axis with specified dimensions
    ax = plt.subplot(111)
    ...
    cbar.set_label('Dust mass density [M$_{\odot}$/kpc$^2$]', fontsize=20)
    ...
    # Add title with smaller font if provided
    if title:
        ax.set_title(title, pad=10)
    
    # Adjust layout to maximize plot area and minimize white space
    plt.tight_layout()
...
def print_summary_statistics(cdm_results, wdm_results):
    ...
    count_equal_dust = sum(np.isclose(cdm_dust_masses, wdm_dust_masses, rtol=1e-4))
...
def main():
    ...
    for i, (cdm_idx, wdm_idx, distance) in enumerate(matches):
        logger.info(f"\nAnalyzing matched halo pair {i+1}/{len(matches)}")
        ...
        try:
            ...
            # Process halo data
            cdm_gas_coords, cdm_gas_mass, cdm_dust_mass, cdm_metallicity = process_halo_data(
            ...
            # Create dust maps
            cdm_output_path = os.path.join(CONFIG['figures_dir'], f'xy_cdm_halo_{i}.png')
            wdm_output_path = os.path.join(CONFIG['figures_dir'], f'xy_wdm_halo_{i}.png')
            
            create_dust_map(
                cdm_gas_coords, cdm_dust_mass, cdm_halo_rad, CONFIG['hist_bin_size'],
                cdm_output_path, f"CDM Dust Distribution - Halo {cdm_idx}"
            )
            
            create_dust_map(
                wdm_gas_coords, wdm_dust_mass, wdm_halo_rad, CONFIG['hist_bin_size'],
                wdm_output_path, f"WDM Dust Distribution - Halo {wdm_idx}"
            )
